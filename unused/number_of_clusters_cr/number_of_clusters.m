...
%NUMBER_OF_CLUSTERS indicates the ideal number of predefined clusters
    ...
    function run_callback(varargin)
        ...
        export_num_of_clusters(segmentation_configs.OUTPUT_DIR,data);
        data = num2cell(data); 
        set(t,'Data',data);
        set(h.graphs,'Enable','on');
        set(t,'UserData',{nc,res1bare,res2bare,res1,res2,res3,covering});
    ...
    function graphs_callback(varargin)
        ...
        try  
            tmp = get(t,'UserData');
            results_clustering_parameters_graphs(segmentation_configs.OUTPUT_DIR,tmp{1},tmp{2},tmp{3},tmp{4},tmp{5},tmp{6},tmp{7});
